            ...
            <section className={`popup__container` + (props.openModal ? ' popup__container_visible' : '')} >
                ...
                <form className='popup__form' onSubmit={e => { e.preventDefault(); getSubmitHandler()(e) }}>
                  ...
                  {modalVersion === 'signup' && (
                    ...
                    {props.modalVersion !== 'success' && (
                      <>
                        <button className={`popup__submit ${isValid ? 'popup__submit_active': ''}`} type='submit' onClick={props.modalVersion === 'signup' ? props.handleSignUp : props.handleLogIn}>Sign {props.modalVersion === 'signup' ? ' up' : ' in'}</button>
